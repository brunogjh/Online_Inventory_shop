...
#If user given creden0tial matches successfully with the data available in database then we will echo string login_success
...
if(isset($_POST["email"]) && isset($_POST["password"])){
	...
	if($count == 1){
			...
			$sql2 = "UPDATE `user_info` SET `last_login` = '$time' WHERE email = '$email'";
					$run_query2 = mysqli_query($con,$sql2);
			//if user is login from page we will send login_success
			echo "login_success";
			$BackToMyPage = "/index.php";
				if(!isset($BackToMyPage)) {
					header('Location: /product.php?p=72#!');
